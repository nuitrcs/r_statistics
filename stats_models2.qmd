...
If you don't already have the data loaded from the previous part of the workshop:
...
The output of `summary()` also lets us know that 2 observations were dropped from the model because they had a missing value for at least one of the variables included in the model.  Observations (rows) must have non-missing values for all variables in the model to be included in the estimation.  
...
Most regression models are more complicated than just two continuous variables.  We can include categorical character variables as predictors in the model as well, and R takes care of converting them into the format needed.  We do not have to create binary indicator variables (or perform one-hot encoding) before running the regression.
...
By default, R takes the lowest category value (alphabetic by default if the variable isn't already a factor) as the base category: it's what's represented by the Intercept.  It then computes how much the intercept needs to shift relative to this baseline for the other categories.  
...
To interpret the full effect of either bill variable on body_mass_g, we would need to take into account multiple coefficients.  
...
```{r}
reg4 <- lm(body_mass_g ~ flipper_length_mm * species, 
   data=penguins)
predict(reg4, data.frame(flipper_length_mm = seq(180, 220, by=5), species = "Chinstrap"))
predict(reg4, data.frame(flipper_length_mm = seq(180, 220, by=5), species = "Gentoo"))
```
...
Working with many different variables can be challenging.  Sometimes, we want to see if our data with multiple dimensions (or variables) can be well summarized or represented by a smaller number of dimensions.  For example, looking at penguins of the same species, we have multiple size measurements of each penguin.  Do we need to use all of these separate measures in our analysis, or could they all be summarized well by a common "size" dimension that explains a large proportion of the variance in the individual variables?

PCA is one technique for dimension reduction of a numerical dataset.  

## Check Variables

For PCA to work well, we want to start with variables with at least reasonable correlation with each other:

```{r}
penguins %>%
  # limit to 1 species because we know there's variance
  filter(species == "Adelie") %>% 
  # select just related numeric variables
  select(body_mass_g, bill_depth_mm, bill_length_mm, flipper_length_mm) %>%
  cor(use="pairwise")
```
...
`center` and `scale.` are important so the variables receive equal weight in the analysis.  

The direct output of `prcomp()` shows how each variable contributes to each dimension -- sort of similar to regression coefficients.  

There will be dimensions computed equal to the number of variables in the analysis.  Each dimension is orthogonal (uncorrelated for centered variables) to the others.  But how many of these are meaningful, i.e. how many dimensions or components explain a substantively significant proportion of the variance among these variables?

We want to start with the output of the `summary()` so we can see the proportion of variance explained by each component.  

```{r}
summary(pca)
```

Here, a single dimension captures over half of the variance in these 4 variables, which is good but not great.  It captures a lot, but there is still a notable amount of additional variance not explained by the first dimension.  

## Number of Dimensions

Depending on why were are trying to reduce the dimensions in our data, we may need to decide how many dimensions we should include as a good summary of the data.  

When we have a lot of features, such as when working with sensor data or sometimes when building machine learning models, we might set a threshold of how much variance we want selected components to capture.  For example, we may select the number of components that together capture 95% of the variance.  

When we're working with a smaller number of variables, we might keep choosing components until the amount of variance explained starts to drop off.  People sometimes look at a scree plot:

```{r}
screeplot(pca, type="lines")
```

Sometimes people look for the bend or elbow in the plot, and only choose components up to the elbow.  Here, that would be the first two components.  


## Extracting Components

Once you choose the number of components, you need to extract them to use as variables in subsequent analysis.

```{r}
comps <- as.data.frame(pca$x[,1:2])

adelie_pca <- penguins %>%
  # select the observations we used for the PCA
  filter(species == "Adelie") %>%
  select(body_mass_g, bill_depth_mm, bill_length_mm, flipper_length_mm) %>%
  filter(complete.cases(.)) %>%
  # add in the PCA components we extracted
  bind_cols(comps)

adelie_pca

plot(adelie_pca$PC1 ~ adelie_pca$PC2)
cor(adelie_pca$PC1, adelie_pca$PC2) # uncorrelated
```
