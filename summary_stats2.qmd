...
None: this is likely the start of a new day/section, so start fresh
...
Note that the values in the `summary()` output are rounded.
...
The `.` after some of the numbers indicates that the values are rounded.
...
Proportions within each species total to 1.
